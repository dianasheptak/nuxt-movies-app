<template>
    <div class="flex flex-col px-20 mt-10">
        <div class="grid grid-cols-7 gap-1">
            ...
            <div class="flex flex-col col-span-3 pl-5">
                <div class="text-4xl font-sans font-bold mb-5">"{{ data?.title }}"</div>
                <div class="flex">
                    <div class="px-4 py-2 bg-gray-200 text-gray-800 rounded-full mr-2 mb-2" v-for="genre in data?.genres" :key="genre.name">{{ genre.name }}</div>
                </div>
...
const {data} = await useFetch<Movie>(`https://api.themoviedb.org/3/movie/${movieId.value}?api_key=716c854015c2536498957711e64da77d`);
